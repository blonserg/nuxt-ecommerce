<template>
  <div class="cart-item">
    <nuxt-link class="cart-product-image_link" :to="`/product/${product.slug}`">
      <div class="cart-product-image_wrap">
        <img
          v-if="product.images"
          v-lazy="product.images.length ? product.images[0] : ''"
    ...
    <div class="cart-product-wrap">
      ...
      <div class="cart-product-price">
        <ProductPrice :price="{ pPrice: product.price.pPrice }" />
      </div>
    </div>
    <div class="cart-product-amount">
      <b-form-spinbutton
        ...
        min="1"
        max="10"
        @change="onQuantityChangeHandler($event, product.id)"
      >
        <svg
          slot="increment"
          hasFocus="true"
    ...
    <div class="cart-product-total">
      <span class="cart-product-total-sub d-md-none">Итого:</span>
      <ProductPrice :price="{ pPrice: product.totalPrice.pPrice}" />
    </div>
    <div class="cart-product-delete">
      <CloseOrDeleteButton
        button-type="delete"
        @click.native="onRemoveClickHandler(product.id)"
      />
    </div>
  </div>
</template>
...
<script>
import CloseOrDeleteButton from '~~/components/common/input/CloseOrDeleteButton'
...
export default {
  ...
  methods: {
    onRemoveClickHandler (productId) {
      this.$store.commit('cart/REMOVE_FROM_CART', productId)
    },
    onQuantityChangeHandler: debounce(function onQuantityChangeHandler (quantity, id) {
      this.$store.commit('cart/UPDATE_PRODUCT_QUANTITY', { id, quantity })
    }, 400)
  }
}
</script>
